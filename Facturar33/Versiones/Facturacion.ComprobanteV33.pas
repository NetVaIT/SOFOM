...
uses xmldom, XMLDoc, XMLIntf,
     ...
     Facturacion.ImpuestosLocalesV1, Facturacion.ComplementoPagoV1;
...
{ IComprobanteFiscalV33_Complemento }
  ...
  IComprobanteFiscalV33_Complemento = interface(IXMLNode)
    ...
    function GetImpuestosLocales: IImpuestosLocalesV1;
    procedure SetImpuestosLocales(const Value: IImpuestosLocalesV1);
    function GetPagos: IPagosV1;
    procedure SetPagos(const Value: IPagosV1);
    property TimbreFiscalDigital: ITimbreFiscalDigitalV33 read GetTimbreFiscalDigital write SetTimbreFiscalDigital;
    property ImpuestosLocales: IImpuestosLocalesV1 read GetImpuestosLocales write SetImpuestosLocales;
    property Pagos: IPagosV1 read GetPagos write SetPagos;
  end;
...
begin
  ...
  RegisterChildNode('ImpuestosLocales', TImpuestosLocalesV1,
                    Facturacion.ImpuestosLocalesV1.TargetNamespace);
  RegisterChildNode('Pagos', TPagosV1,
                    Facturacion.ComplementoPagoV1.TargetNamespace);
  inherited;
end;

function TComprobanteFiscalV33_Complemento.GetImpuestosLocales: IImpuestosLocalesV1;
begin
  if (Self.HasChildNode('ImpuestosLocales', Facturacion.ImpuestosLocalesV1.TargetNamespace)) then
    Result := ChildNodes.FindNode('ImpuestosLocales', Facturacion.ImpuestosLocalesV1.TargetNamespace) as IImpuestosLocalesV1
  else
    Result := nil;
end;

function TComprobanteFiscalV33_Complemento.GetPagos: IPagosV1;
begin
  if (Self.HasChildNode('Pagos', Facturacion.ComplementoPagoV1.TargetNamespace)) then
    Result := ChildNodes.FindNode('Pagos', Facturacion.ComplementoPagoV1.TargetNamespace) as IPagosV1
  else
    Result := nil;
end;
...
procedure TComprobanteFiscalV33_Complemento.SetImpuestosLocales(
  const Value: IImpuestosLocalesV1);
begin
  ChildNodes.Add(Value);
end;

procedure TComprobanteFiscalV33_Complemento.SetPagos(const Value: IPagosV1);
begin
  ChildNodes.Add(Value);
end;
