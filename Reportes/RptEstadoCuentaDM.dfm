inherited dmRptEstadoCuenta: TdmRptEstadoCuenta
  Height = 383
  Width = 346
  ...
  object adoqEstadoCuenta: TADOQuery
    ...
    SQL.Strings = (
      'SET LANGUAGE spanish;'
        ...
        'SELECT        CXC.IdCuentaXCobrar, CXC.IdPersona, CXC.IdAnexo, C' +
        'XC.IdAnexosAmortizaciones, Contrato.IdContratoTipo,'
      
        'dbo.GetDateAux() AS Fecha, Emisor.Emisor, Emisor.Direccion, Pers' +
        'ona.Cliente, Persona.RFC, Persona.CURP, Persona.BancoDeposito, P' +
        'ersona.CuentaDeposito, Persona.CLABEDeposito,'
      
        'Contrato.Contrato, Contrato.Anexo, Contrato.MontoFinanciar, Cont' +
        'rato.SaldoInsoluto, Contrato.TasaAnual, Contrato.TasaMoratoriaAn' +
        'ual, Contrato.TipoContrato, FechaVencimientoProximo, SaldoProxim' +
        'o, P.FechaUltimoPago,'
      
        'CFDI.Serie+CAST(CFDI.Folio AS Varchar(10)) AS FolioFactura, dbo.' +
        'GetAnexoAmortizacionIdentificador(CXC.IdAnexosAmortizaciones) AS' +
        ' Periodo, CXC.FechaVencimiento,'
      
        'CASE WHEN CXC.IdAnexosAmortizaciones IS NULL THEN CXC.Descripcio' +
        'n ELSE '#39'Amortizaci'#243'n '#39' + DATENAME(month,CXC.FechaVencimiento) + ' +
        #39' '#39' + DATENAME(year,CXC.FechaVencimiento) END AS Descripcion,'
      
        'ISNULL(CXCMora.Dias,0) Dias, ISNULL(CXC.Saldo,0) AS SaldoAmortiz' +
        'acion, ISNULL(CXCMora.Saldo,0) AS SaldoMoratorios,'
      'ISNULL(CXC.Saldo,0)+ISNULL(CXCMora.Saldo,0) AS Total'
      'FROM            CuentasXCobrar AS CXC'
      'LEFT OUTER JOIN CFDI ON CXC.IdCFDI = CFDI.IdCFDI'
      'LEFT OUTER JOIN'
      
        '    (SELECT CXCM.IdCuentaXCobrarBase, SUM(AM.Dias) AS Dias, SUM(' +
        'CXCM.Saldo) AS Saldo'
      '    FROM CuentasXCobrar AS CXCM LEFT OUTER JOIN'
      '        (SELECT        IdCuentaXCobrar, COUNT(*) AS Dias'
      '        FROM            AnexosMoratorios'
      '        WHERE        (IdAnexoMoratorioEstatus = 1)'
      
        '        GROUP BY IdCuentaXCobrar) AS AM ON AM.IdCuentaXCobrar = ' +
        'CXCM.IdCuentaXCobrar'
      #9'WHERE CXCM.EsMoratorio = 1 AND CXCM.Saldo > 0.01'
      
        #9'GROUP BY CXCM.IdCuentaXCobrarBase) AS CXCMora ON CXCMora.IdCuen' +
        'taXCobrarBase = CXC.IdCuentaXCobrar'
      'LEFT OUTER JOIN'
      '    (SELECT IdAnexo, MAX(FechaPago) AS FechaUltimoPago'
      '    FROM Pagos'
      '    GROUP BY IdAnexo) AS P ON P.IdAnexo = CXC.IdAnexo'
      'LEFT OUTER JOIN'
      
        #9'(SELECT Personas.IdPersona, Personas.RazonSocial AS Cliente, Pe' +
        'rsonas.RFC, Personas.CURP, Bancos.Nombre AS BancoDeposito, Perso' +
        'nas.CuentaDeposito, Personas.CLABEDeposito'
      #9'FROM Personas'
      
        #9'LEFT OUTER JOIN Bancos ON Personas.IdBancoDeposito = Bancos.IdB' +
        'anco) AS Persona ON Persona.IdPersona = CXC.IdPersona'
      'LEFT OUTER JOIN'
      
        #9'(SELECT Anexos.IdAnexo, ContratosTipos.IdContratoTipo, Contrato' +
        's.Identificador AS Contrato, Anexos.Identificador As Anexo, Anex' +
        'os.MontoFinanciar, Anexos.SaldoInsoluto, Anexos.TasaAnual, Anexo' +
        's.TasaMoratoriaAnual, ContratosTipos.Descripcion AS TipoContrato' +
        ','
      
        #9'(SELECT FechaVencimiento from AnexosAmortizaciones WHERE IdAnex' +
        'oAmortizacion = dbo.GetAmortizacionproximoVencimiento(Anexos.IdA' +
        'nexo,dbo.GetDateAux())) AS FechaVencimientoProximo,'
      
        #9'(SELECT PagoTotal from AnexosAmortizaciones WHERE IdAnexoAmorti' +
        'zacion = dbo.GetAmortizacionproximoVencimiento(Anexos.IdAnexo,db' +
        'o.GetDateAux())) AS SaldoProximo'
      #9'FROM Anexos'
      
        #9'INNER JOIN Contratos ON Anexos.IdContrato = Contratos.IdContrat' +
        'o'
      
        #9'INNER JOIN ContratosTipos ON Contratos.IdContratoTipo = Contrat' +
        'osTipos.IdContratoTipo) AS Contrato ON Contrato.IdAnexo = CXC.Id' +
        'Anexo,'
      
        #9'(SELECT Personas.RazonSocial AS Emisor, ISNULL(Domicilios.Domic' +
        'ilio, '#39#39') + '#39' * Tel. '#39' + ISNULL(Telefonos.Lada + '#39' '#39' + Telefonos' +
        '.Telefono, '#39#39') AS Direccion'
      #9'FROM Personas'
      
        #9'LEFT OUTER JOIN Telefonos ON Personas.IdPersona = Telefonos.IdP' +
        'ersona AND Telefonos.IdTelefonoTipo = 4 AND Telefonos.Predetermi' +
        'nado = 1'
      #9'LEFT OUTER JOIN'
      
        #9#9'(SELECT        PersonasDomicilios.IdPersona, Domicilios.Calle ' +
        '+ '#39' '#39' + ISNULL(Domicilios.NoExterior, '#39#39') + '#39' '#39' + ISNULL(Domicil' +
        'ios.NoInterior, '#39#39') + '#39' * Col. '#39' + ISNULL(Domicilios.Colonia, '#39#39 +
        ')'
      
        #9#9#9#9#9#9#9#9#9'+ '#39' * '#39' + Municipios.Descripcion + '#39' * '#39' + Estados.Desc' +
        'ripcion + '#39' * CP '#39' + Domicilios.CodigoPostal + '#39' * '#39' + Paises.De' +
        'scripcion AS Domicilio'
      #9#9'FROM            PersonasDomicilios INNER JOIN'
      
        #9#9#9#9#9#9#9#9#9'Domicilios ON PersonasDomicilios.IdDomicilio = Domicili' +
        'os.IdDomicilio INNER JOIN'
      #9#9#9#9#9#9#9#9#9'Paises ON Domicilios.IdPais = Paises.IdPais INNER JOIN'
      
        #9#9#9#9#9#9#9#9#9'Estados ON Domicilios.IdEstado = Estados.IdEstado INNER' +
        ' JOIN'
      
        #9#9#9#9#9#9#9#9#9'Municipios ON Domicilios.IdMunicipio = Municipios.IdMun' +
        'icipio INNER JOIN'
      
        #9#9#9#9#9#9#9#9#9'Poblaciones ON Domicilios.IdPoblacion = Poblaciones.IdP' +
        'oblacion) AS Domicilios ON Personas.IdPersona = Domicilios.IdPer' +
        'sona'
  ...
  object dsEstadoCuenta: TDataSource
    AutoEdit = False
    DataSet = adoqEstadoCuenta
    Left = 200
    Top = 176
  end
  object ppRptEstadoCuenta: TppReport
    AutoStop = False
    ...
    object ppHeaderBandEstadoCuenta: TppHeaderBand
      BeforePrint = ppHeaderBandEstadoCuentaBeforePrint
      Background.Brush.Style = bsClear
      mmBottomOffset = 0
      mmHeight = 82021
      mmPrintPosition = 0
      ...
      object pplblTituloEstadoCuenta: TppLabel
        UserName = 'Titulo'
        AutoSize = False
        Caption = 'ESTADO DE CUENTA EN MONEDA NACIONAL "PESOS"'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Name = 'Arial'
        Font.Size = 16
      ...
      object lblMontaFinanciar: TppLabel
        ...
        Caption = 'Importe del cr'#233'dito'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
      ...
      object dbtxtSaldoInsoluto: TppDBText
        UserName = 'dbtxtSaldoInsoluto'
        AutoSize = True
      ...
      object ppLabel87: TppLabel
        UserName = 'Label66'
        Caption = #218'ltimo Pago Recibido'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
    ...
    object ppGroup3: TppGroup
      ...
      object ppGroupFooterBand3: TppGroupFooterBand
        ...
        object ppLabel88: TppLabel
          UserName = 'Label67'
          AutoSize = False
          Caption = 'Datos Bancarios para el Dep'#243'sito'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
        ...
        object ppLabel100: TppLabel
          UserName = 'Label100'
          Caption = 'Pr'#243'ximo Vencimiento'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
        ...
        object ppDBText76: TppDBText
          UserName = 'DBText76'
          DataField = 'FechaVencimientoProximo'
          DataPipeline = ppdbpEstadoCuenta
          DisplayFormat = 'dd '#39'de'#39' mmmm '#39'del '#39'yyyy'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
  ...
  object ppdbpEstadoCuenta: TppDBPipeline
    DataSource = dsEstadoCuenta
    UserName = 'EstadoCuenta'
    Left = 104
    Top = 240
  end
  object adopGenTmpEstadoCuenta: TADOStoredProc
    Connection = _dmConection.ADOConnection
    ProcedureName = 'p_GenTmpEstadoCuenta;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
  ...
  object adoqEstadoCuentaFuturo: TADOQuery
    Connection = _dmConection.ADOConnection
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    Parameters = <
    ...
    SQL.Strings = (
      ...
      'SET LANGUAGE spanish;'
        ...
        '@Fecha AS Fecha, Emisor.Emisor, Emisor.Direccion, Persona.Client' +
        'e, Persona.RFC, Persona.CURP, Persona.BancoDeposito, Persona.Cue' +
        'ntaDeposito, Persona.CLABEDeposito,'
      ...
      'LEFT OUTER JOIN'
        ...
        #9'(SELECT FechaVencimiento from AnexosAmortizaciones WHERE IdAnex' +
        'oAmortizacion = dbo.GetAmortizacionproximoVencimiento(Anexos.IdA' +
        'nexo,@Fecha)) AS FechaVencimientoProximo,'
      
        #9'(SELECT PagoTotal from AnexosAmortizaciones WHERE IdAnexoAmorti' +
        'zacion = dbo.GetAmortizacionproximoVencimiento(Anexos.IdAnexo,@F' +
        'echa)) AS SaldoProximo'
      #9'FROM Anexos'
